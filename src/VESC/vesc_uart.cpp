...
#ifdef DEBUG
    DEBUGSERIAL.print("SRC received: ");
    DEBUGSERIAL.println(crcMessage);
#endif // DEBUG
...
#ifdef DEBUG
    DEBUGSERIAL.print("SRC calc: ");
    DEBUGSERIAL.println(crcPayload);
#endif
...
#ifdef DEBUG
    DEBUGSERIAL.print("UART package send: ");
...
bool process_read_package(uint8_t *message, mc_values &values, int len)
{
    ...
    case COMM_GET_VALUES:
        ...
        values.temp_motor = buffer_get_float16(message, 10.0, &ind);
...
void serial_print(const mc_values &values)
{
    ...
    DEBUGSERIAL.print("Temp MOS1: ");
    DEBUGSERIAL.println(values.temp_mos1);
    DEBUGSERIAL.print("Temp MOS2: ");
    DEBUGSERIAL.println(values.temp_mos2);
    DEBUGSERIAL.print("Temp MOS3: ");
    DEBUGSERIAL.println(values.temp_mos3);
    DEBUGSERIAL.print("Temp MOS4: ");
    DEBUGSERIAL.println(values.temp_mos4);
    DEBUGSERIAL.print("Temp MOS5: ");
    DEBUGSERIAL.println(values.temp_mos5);
    DEBUGSERIAL.print("Temp MOS6: ");
    DEBUGSERIAL.println(values.temp_mos6);
    DEBUGSERIAL.print("Temp PCB: ");
    DEBUGSERIAL.println(values.temp_pcb);

    DEBUGSERIAL.print("avgMotorCurrent: ");
    DEBUGSERIAL.println(values.current_motor);
    DEBUGSERIAL.print("avgInputCurrent: ");
    DEBUGSERIAL.println(values.current_in);
    DEBUGSERIAL.print("dutyCycleNow: ");
    DEBUGSERIAL.println(values.duty_now);
    DEBUGSERIAL.print("rpm: ");
    DEBUGSERIAL.println(values.rpm);
    DEBUGSERIAL.print("inputVoltage: ");
    DEBUGSERIAL.println(values.v_in);
    DEBUGSERIAL.print("ampHours: ");
    DEBUGSERIAL.println(values.amp_hours);
    DEBUGSERIAL.print("ampHoursCharges: ");
    DEBUGSERIAL.println(values.amp_hours_charged);
    DEBUGSERIAL.print("tachometer: ");
    DEBUGSERIAL.println(values.tachometer);
    DEBUGSERIAL.print("tachometerAbs: ");
    DEBUGSERIAL.println(values.tachometer_abs);
    DEBUGSERIAL.print("Fault Code: ");
    DEBUGSERIAL.println(values.fault_code);
}
...
void set_nunchuck_values(nunchuckPackage *package){
	...
	payload[ind++] = package->valueX;
	payload[ind++] = package->valueY;
	buffer_append_bool(payload, package->lowerButton, &ind);
	buffer_append_bool(payload, package->upperButton, &ind);
	...
	//if(DEBUGSERIAL != NULL){
		DEBUGSERIAL.println("Data reached at setNunchuckValues:");
		DEBUGSERIAL.print("valueX = "); DEBUGSERIAL.print(package->valueX); DEBUGSERIAL.print(" valueY = "); DEBUGSERIAL.println(package->valueY);
		DEBUGSERIAL.print("LowerButton = "); DEBUGSERIAL.print(package->lowerButton); DEBUGSERIAL.print(" UpperButton = "); DEBUGSERIAL.println(package->upperButton);
	//}

	send_payload(payload, 11);
}