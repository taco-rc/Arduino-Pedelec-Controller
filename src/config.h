...
#define NUM_CELLS 0
#define NUM_CELLS_REAL 10
//Config Options-----------------------------------------------------------------------------------------------------
const int pas_tolerance=1;               //0... increase to make pas sensor slower but more tolerant against speed changes
const int throttle_offset=267;           //Offset voltage of throttle control when in "0" position (0..1023 = 0..5V)
const int throttle_max=1023;              //Offset voltage of throttle control when in "MAX" position (0..1023 = 0..5V)
...
const int motor_max=1000;                  //Maximum input value for motor driver (0..255 = 0..5V). Default: 200. In Servo mode this value is about 2000
...
const float wheel_circumference = 0.98; //wheel circumference in m
...
const int power_max=250;                 //Maximum power in W (throttle mode)
const int power_poti_max=250;            //Maximum power in W (poti mode) or maximum percentage of human power drawn by motor (torque mode)
const int thermal_limit=100;             //Maximum continuous thermal load motor can withstand
...
const double capacity = 2 * NUM_CELLS_REAL * 3.7;           //battery capacity in watthours for range calculation
const double pas_factor_min=0.3;         //Use pas_factor from hardware-test here with some tolerances. Both values have to be eihter larger or smaller than 1.0 and not 0.0!
const double pas_factor_max=0.6;           //Use pas_factor from hardware-test here with some tolerances. Both values have to be eihter larger or smaller than 1.0 and not 0.0!
...
const double cfg_pid_i=1.5;              //pid i-value, default: 2.0
